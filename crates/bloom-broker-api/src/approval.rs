...
mod tests {
    ...
    /// This crate owns the authoritative Solana golden vector, so the
    /// normalizer is checked against the same constants the Broker verifier
    /// and the Machine constructor consume.
    use crate::solana_vectors::{MESSAGE_DIGEST_HEX, MESSAGE_HEX};

    fn golden_message() -> Vec<u8> {
        (0..MESSAGE_HEX.len() / 2)
            .map(|index| u8::from_str_radix(&MESSAGE_HEX[index * 2..index * 2 + 2], 16).unwrap())
            .collect()
    }

...
pub(crate) fn update_to_signer(value: north::PolicyUpdateRequest) -> south::PolicyUpdateRequest {
    south::PolicyUpdateRequest {
        operation_id: value.operation_id,
        wallet_id: value.wallet_id,
        baseline_version: value.baseline_version,
        baseline_digest: value.baseline_digest,
        proposed_canonical_policy: value.proposed_canonical_policy,
        proposed_policy_digest: value.proposed_policy_digest,
...
mod tests {
    ...
    fn update_snapshot_and_commit_fields_are_preserved() {
        let update = north::PolicyUpdateRequest {
            ...
            proposed_canonical_policy: north::Base64UrlBytes::from_bytes(&[5]),
            proposed_policy_digest: digest(6),
            authority_diff_digest: digest(7),
            assurance_level: north::Token::new("assurance-8").unwrap(),
        };
        let mapped_update = update_to_signer(update);
        assert_eq!(
        ...
        assert_eq!(mapped_update.proposed_canonical_policy.decode(), vec![5]);
        assert_eq!(mapped_update.proposed_policy_digest, digest(6));
